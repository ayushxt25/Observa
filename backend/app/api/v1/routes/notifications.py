...
def create_channel(payload: NotificationChannelCreate, request: Request, repo: Annotated[NotificationRepository, Depends(get_notification_repository)], membership: Annotated[WorkspaceMembershipModel, Depends(require_workspace_role("admin"))], settings: Annotated[Settings, Depends(get_settings)]) -> NotificationChannelOut:
    ...
    channel = repo.create_channel(membership.workspace_id, payload, commit=False)
    AuditService(repo.db).record_user(membership=membership, action=audit_actions.NOTIFICATION_CHANNEL_CREATED, resource_type="notification_channel", resource_id=channel.id, request=request, metadata={"name": channel.name, "type": channel.type, "enabled": channel.enabled}, commit=False)
    repo.db.commit()
    repo.db.refresh(channel)
...
def update_channel(channel_id: str, payload: NotificationChannelPatch, request: Request, repo: Annotated[NotificationRepository, Depends(get_notification_repository)], membership: Annotated[WorkspaceMembershipModel, Depends(require_workspace_role("admin"))], settings: Annotated[Settings, Depends(get_settings)]) -> NotificationChannelOut:
    channel = repo.get_channel(membership.workspace_id, channel_id)
    if channel is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Notification channel not found")
    if channel.type == "webhook" and payload.webhook_config is not None:
        try:
            ...
            raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail=str(exc)) from exc
    before = {"name": channel.name, "type": channel.type, "enabled": channel.enabled, "hasSecret": channel.secret_encrypted is not None}
    updated = repo.update_channel(channel, payload, commit=False)
    after = {"name": updated.name, "type": updated.type, "enabled": updated.enabled, "hasSecret": updated.secret_encrypted is not None}
    AuditService(repo.db).record_user(membership=membership, action=audit_actions.NOTIFICATION_CHANNEL_UPDATED, resource_type="notification_channel", resource_id=channel_id, request=request, metadata=changed_fields(before, after), commit=False)
    repo.db.commit()
    repo.db.refresh(updated)
    return channel_out(updated)
...
def delete_channel(channel_id: str, request: Request, repo: Annotated[NotificationRepository, Depends(get_notification_repository)], membership: Annotated[WorkspaceMembershipModel, Depends(require_workspace_role("admin"))]) -> None:
    channel = repo.get_channel(membership.workspace_id, channel_id)
    if channel is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Notification channel not found")
    metadata = {"name": channel.name, "type": channel.type, "enabled": channel.enabled}
    repo.delete_channel(channel, commit=False)
    AuditService(repo.db).record_user(membership=membership, action=audit_actions.NOTIFICATION_CHANNEL_DELETED, resource_type="notification_channel", resource_id=channel_id, request=request, metadata=metadata, commit=False)
    repo.db.commit()
...
def test_channel(channel_id: str, request: Request, repo: Annotated[NotificationRepository, Depends(get_notification_repository)], membership: Annotated[WorkspaceMembershipModel, Depends(require_workspace_role("admin"))], settings: Annotated[Settings, Depends(get_settings)]) -> TestNotificationResponse:
    ...
    if channel is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Notification channel not found")
    delivery = repo.create_test_delivery(membership.workspace_id, channel, commit=False)
    AuditService(repo.db).record_user(membership=membership, action=audit_actions.NOTIFICATION_CHANNEL_TESTED, resource_type="notification_channel", resource_id=channel.id, request=request, metadata={"deliveryId": delivery.id, "type": channel.type}, commit=False)
    repo.db.commit()
...
def set_alert_channels(rule_id: str, payload: AlertChannelUpdate, request: Request, alerts: Annotated[AlertRepository, Depends(get_alert_repository)], repo: Annotated[NotificationRepository, Depends(get_notification_repository)], membership: Annotated[WorkspaceMembershipModel, Depends(require_workspace_role("member"))]) -> AlertChannelUpdate:
    if alerts.get_rule(rule_id, workspace_id=membership.workspace_id) is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Alert rule not found")
    try:
        channel_ids = repo.set_alert_channels(membership.workspace_id, rule_id, payload.channel_ids, commit=False)
        AuditService(repo.db).record_user(membership=membership, action=audit_actions.ALERT_NOTIFICATION_CHANNELS_UPDATED, resource_type="alert_rule", resource_id=rule_id, request=request, metadata={"channelIds": channel_ids}, commit=False)
        repo.db.commit()
        return AlertChannelUpdate(channel_ids=channel_ids)
